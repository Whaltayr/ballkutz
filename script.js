...
const LOCALES = {
  pt: {
    ...
    f_srv_kutz_crianca: "Kutz Criança (Sub 15) — 4.000 KZ",
    f_srv_kutz_cabelo: "Kutz de Cabelo — 5.000 KZ",
    f_srv_kutz_senior: "Kutz Seniores (+65) — 5.000 KZ",
    f_srv_ritual_barba: "Ballkutz Ritual (Barboterapia) — 6.000 KZ",
    f_srv_kutz_barba: "Kutz de Barba — 4.000 KZ",
    f_srv_cabelo_barba: "Kutz de Cabelo + Barba — 8.000 KZ",
    ...
    f_srv_limpeza: "Limpeza Facial — 5.000 KZ",
    ...
    f_srv_24k_limpeza: "24K Gold Limpeza de Pele — 10.000/13.000 KZ ",
    ...
    about_p1:
      "A BallKutz surgiu da ideia de juntar o mundo do desporto (futebol e basquetebol), elevando a autoestima do seu cliente ao mais alto nível. Um espaço onde cada visita é uma experiência única — música, jogos de PlayStation e o melhor corte da tua vida.",
    about_p2:
    ...
    plan1_name: "KUTZ Cabelo (2x/mês - Limitado) — 7.500 KZ",
    plan2_name: "KUTZ Cabelo + Barba (2x/mês - Limitado) — 10.500 KZ",
    plan3_name: "KUTZ Cabelo  — 10.000 KZ",
    plan4_name: "KUTZ Barba — 8.500 KZ",
    plan5_name: "KUTZ Criança — 7.500 KZ",
    plan6_name: "KUTZ Cabelo + Barba — 15.000 KZ",
  ...
  home_srv_notice_text: "To ensure the best experience in the comfort of your space, the prices below are exclusive to home services. The barber's transportation fee (taxi/ride app) is the client's sole responsibility.",
    ...
    f_srv_limpeza: "Facial Cleansing — 5.000 KZ",
    ...
    f_srv_24k_limpeza: "24K Gold Skin Cleansing — 10.000/13.000 KZ KZ",
  ...
  home_srv_notice_text: "Pour garantir la meilleure expérience dans le confort de votre espace, les tarifs ci-dessous sont exclusifs aux services à domicile. Les frais de transport du coiffeur (taxi/VTC) sont à l'entière charge du client.",
    ...
    f_srv_limpeza: "Nettoyage du visage — 5.000 KZ",
    ...
    f_srv_24k_limpeza: "Nettoyage Or 24K — 10.000/13.000 KZ",
  ...
  home_srv_notice_text: "Para garantizar la mejor experiencia en la comodidad de su espacio, los precios a continuación son exclusivos para servicios a domicilio. La tarifa de transporte del barbero (taxi/app) es responsabilidad exclusiva del cliente.",
    ...
    f_srv_limpeza: "Limpieza Facial — 5.000 KZ",
    ...
    f_srv_24k_limpeza: "Limpieza Oro 24K — 10.000/13.000 KZ ",
  ...
  home_srv_notice_text: "为了在您舒适的空间内提供最佳体验，以下价格为上门服务专属。理发师的交通费用（出租车/网约车）由客户全额承担。",
    ...
    f_srv_24k_limpeza: "24K金皮肤清洁 — 10.000/13.000 KZ",
...
// ── Time slots (CORRIGIDO COM SEGUNDA FECHADA) ──
function renderTimeSlots() {
  ...
  const now = new Date();
  let day = now.getDay(); // 0 = Domingo, 1 = Segunda, etc.
  let isToday = true;
  ...
  TIME_SLOTS.forEach((t) => {
    ...
    // REGRA 1: Bloquear horários passados se a marcação for para hoje
    if (isToday && (hour < now.getHours() || (hour === now.getHours() && minute <= now.getMinutes()))) {
      btn.classList.add("disabled");
      btn.disabled = true;
    }

    // REGRA 2: Domingo desativar horários depois das 17:30
    if (day === 0 && slotMinutes > (17 * 60 + 30)) {
      btn.classList.add("disabled");
      btn.disabled = true;
    }

    // REGRA 3: Segunda-feira FECHADO (bloqueia todos os horários)
    if (day === 1) {
      btn.classList.add("disabled");
      btn.disabled = true;
    }
...
function renderHomeTimeSlots() {
  ...
  if (!grid || !hidden || !dateInput) return;
  ...
  TIME_SLOTS.forEach((t) => {
    ...
    // REGRA 1: Bloquear passados
    if (isToday && (hour < now.getHours() || (hour === now.getHours() && minute <= now.getMinutes()))) {
      btn.classList.add("disabled");
      btn.disabled = true;
    }

    // REGRA 2: Domingo fechar às 17:30
    if (day === 0 && slotMinutes > (17 * 60 + 30)) {
      btn.classList.add("disabled");
      btn.disabled = true;
    }
...
function validateShopForm() {
  ...
  const e = document.getElementById("clientEmail").value.trim(); // <-- CAPTURA O EMAIL
  ...
  showErr("emailError", !isEmail(e)); // <-- ATIVA O ERRO SE O EMAIL ESTIVER VAZIO OU INVÁLIDO
  showErr("serviceError", !s);
  showErr("dateError", !isDate(d));
  showErr("timeError", !t);
  showErr("consentError", !c);

  // O formulário só avança se TODAS as condições forem verdadeiras (incluindo o email)
  return n && isPhone(p) && isEmail(e) && s && isDate(d) && t && c;
}
...
function waHome(d) {
  const lines = [
    ...
    "",
    "✂️ *Serviço:* " + d.service,
    "👤 *Barbeiro:* " + (d.barber || "Qualquer disponível"), // <-- NOVA LINHA AQUI
...
// ── Booking form init ──
// ── Booking form init (CORRIGIDO) ──
function initBookingForm() {
  const now = new Date();
  // Formatar data para YYYY-MM-DD mantendo o fuso local seguro
  const todayStr = now.getFullYear() + "-" + 
                   String(now.getMonth() + 1).padStart(2, '0') + "-" + 
                   String(now.getDate()).padStart(2, '0');
...
//      if (!el.value) el.value = todayStr; // Define a data de hoje por defeito
      ...
      el.addEventListener("change", () => {
        if (id === "bookingDate") renderTimeSlots();
        if (id === "homeDate") renderHomeTimeSlots();
      });
    }
  });
...
// --- SUBMIT DOMICÍLIO ---
  document.getElementById("submitHomeBooking").addEventListener("click", async () => {
    ...
    const data = {
      ...
      barber: document.getElementById("homeBarber").value, // <-- NOVO CAMPO
    ...
    // 2. Enviar para o Make (Webhook) com o barbeiro incluído
    await notifyEmail({
      Nome: data.name,
      WhatsApp: data.phone,
      Email: data.email,
      Serviço: data.service,
      Barbeiro: data.barber || "Qualquer", // <-- NOVO CAMPO
      Data: data.date,
      Hora: data.time,
